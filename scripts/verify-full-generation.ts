...
async function callOpenAICompatible(
  ...
  if (!message)
    throw new Error('LLM message missing')
...
async function main() {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    fullHierarchyResult = parser.extractFullHierarchyResultFromText(result.response, fullHierarchyResult.rootText)
    context.hierarchyDraftText = [
      `- ${fullHierarchyResult.rootText}`,
      ...fullHierarchyResult.hierarchicalItems.map((item) => `${'  '.repeat(item.level + 1)}- ${item.text}`),
    ].join('\n')
    ...
    if (fullHierarchyResult.hierarchicalItems.filter((item) => item.level === 0).length >= 2 && deepestLevel >= 2)
      break
  }
  ...
  if (topLevelItems.length < 2 || deepestLevel < 2) {
    const outlineFallbackResult = buildHierarchyFromSpecificationOutline(specificationText, fullHierarchyResult.rootText)
  ...
  const validatedDeepestLevel = validatedHierarchicalItems.reduce((max, item) => Math.max(max, item.level), 0)

...
const NodeSelector_1 = require("../utils/NodeSelector");
...
const createNewNode = (parentId, order, depth) => ({
    ...
    x: 0,
    y: 0,
    width: NodeSettings_1.MIN_WIDTH,
    height: NodeSettings_1.NODE_HEIGHT,
    section1Height: NodeSettings_1.DEFAULT_SECTION_HEIGHT,
    section2Height: NodeSettings_1.DEFAULT_SECTION_HEIGHT,
    section3Height: NodeSettings_1.DEFAULT_SECTION_HEIGHT,
    parentId,
    order,
    depth,
    children: 0,
...
exports.initialState = {
    elements: {
        1: {
            ...
            id: 1,
            x: NodeSettings_1.DEFAULT_X,
            y: NodeSettings_1.DEFAULT_Y,
            editing: false,
        }
    },
...
const deleteNodeRecursive = (elements, deleteElement) => {
    if (deleteElement.parentId === null)
        return elements;
    const updatedNodes = { ...elements };
    // 子ノードを再帰的に削除
...
const adjustNodeAndChildrenPosition = (elements, element, currentY, maxHeight, visited = new Set()) => {
    ...
    if (!parentNode) {
        element.x = NodeSettings_1.DEFAULT_X;
    }
    else {
        element.x = parentNode.x + parentNode.width + NodeSettings_1.X_OFFSET;
    }
    element.y = currentY;
    ...
    if (childNodes.length > 0) {
        ...
        if (element.visible || element.order === 0) {
            currentY += maxHeight + NodeSettings_1.Y_OFFSET;
        }
    }
    return currentY;
};
const adjustNodePositions = (elements) => {
    ...
    rootNodes.forEach(rootNode => {
        adjustNodeAndChildrenPosition(updatedNodes, rootNode, NodeSettings_1.PRESET_Y, rootNode.height, new Set());
    });
...
const actionHandlers = {
    NEW: () => {
        (0, undoredo_1.clearSnapshots)();
        return exports.initialState;
    },
    ZOOM_IN: handleZoomIn,
    ZOOM_OUT: handleZoomOut,
    ARROW_UP: handleArrowAction(NodeSelector_1.handleArrowUp),
    ARROW_DOWN: handleArrowAction(NodeSelector_1.handleArrowDown),
    ARROW_RIGHT: handleArrowAction(NodeSelector_1.handleArrowRight),
    ARROW_LEFT: handleArrowAction(NodeSelector_1.handleArrowLeft),
    ...
    DELETE_NODE: state => handleNodeMutation(state, (elements, selectedElement) => adjustNodePositions(deleteNodeRecursive(elements, selectedElement))),
    ...
    END_EDITING: state => ({
        ...state,
        elements: adjustNodePositions(Object.values(state.elements).reduce((acc, element) => {
            acc[element.id] = { ...element, editing: false };
            return acc;
        }, {}))
    }),
    UNDO: state => ({ ...state, elements: (0, undoredo_1.Undo)(state.elements) }),
    REDO: state => ({ ...state, elements: (0, undoredo_1.Redo)(state.elements) }),
    SNAPSHOT: state => { (0, undoredo_1.saveSnapshot)(state.elements); return state; },
    DROP_NODE: (state, action) => {
        ...
        return {
            ...state,
            elements: adjustNodePositions(setDepthRecursive({ ...updatedNodes, ...parentUpdates }, payload))
        };
    },
    ...
    EXPAND_NODE: state => handleNodeMutation(state, (elements, selectedNode) => adjustNodePositions(setVisibilityRecursive(elements, selectedNode, true))),
    COLLAPSE_NODE: state => handleNodeMutation(state, (elements, selectedNode) => adjustNodePositions(setVisibilityRecursive(elements, selectedNode, false))),
...
function handleNodeMutation(state, mutationFn) {
    ...
    if (!selectedNode)
        return state;
    (0, undoredo_1.saveSnapshot)(state.elements);

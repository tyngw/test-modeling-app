...
const ModalWindow: React.FC<ModalWindowProps> = ({
  ...
  useEffect(() => {
    ...
    // debugLog(`[DEBUG] ModalWindow(${modalId}) rendered #${renderCount.current}, isOpen=${isOpen}`);
  });
  ...
  useEffect(() => {
    ...
    if (isOpen && !wasOpenRef.current) {
      // debugLog(`[DEBUG] ModalWindow(${modalId}) first opened`);
      if (onOpen) {
        onOpen();
      }
    }
    ...
    if (isOpen) {
      // debugLog(`[DEBUG] ModalWindow(${modalId}) opened (animation)`);
      setIsAnimating(true);
    }

    return () => {
      if (isOpen) {
        // debugLog(`[DEBUG] ModalWindow(${modalId}) cleanup while open`);
      }
    };

...
export const generateWithGemini = async (prompt: string, apiKey: string, modelType: string): Promise<string> => {
    try {
        console.log('prompt: \n', prompt)
            ...
            endpoint,
            {
                ...
                generationConfig: {
                    ...
                    responseSchema: {
                        type: "string",
                        format: "text/plain",
                        description: "A list of items, each on a new line. Do not include bullet points or numbering."
                    }
                }
            },
            {
...
export const generateElementSuggestions = async (
    ...
    try {
        ...
        // JSONレスポンスの取得
        const jsonText = response.data.candidates?.[0]?.content?.parts?.[0]?.text || '{"suggestions":[]}';

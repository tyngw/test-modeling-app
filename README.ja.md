...
- `名前を付けて保存`: 図のデータをJSON形式またはMarkdown形式（VSCode拡張使用時）で保存します
...
- **リアルタイム保存**: 編集内容は自動的にJSONファイルまたはMarkdownファイルとして保存されます
- **ファイル履歴**: VSCodeのファイル履歴機能を利用できます
- **複数形式対応**: JSON形式（完全なデータモデル）とMarkdown形式（階層テキスト形式）の両方をサポート
